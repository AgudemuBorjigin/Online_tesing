...
def sortData(subjList, data, ilds):
    ...
    subjNames = []
    data_fit = np.zeros((len(subjIndexList), len(ilds), 3)) # len(snrs)->num of conds;len(snrs[0])->
                                                            # num of snr; 3 columns: snrs, num of corrects, num of trials
    # for populaiton across subjects
    counters_all = np.zeros((len(ilds),), dtype=int)
    corrects_all = np.zeros((len(ilds),), dtype=int)
    # data extraction
    for si, subj_num in enumerate(subjIndexList):
        # data for a single subject
        data_subj = data[subj_num]
        # temporary variables 
        counters_single = np.zeros((len(ilds),), dtype=int)
        corrects_single = np.zeros((len(ilds),), dtype=int)
        ...
        for i, data_trial in enumerate(data_subj[idx_first_trial:]):
            if data_trial['button_pressed']:
                ...
                index = find(annot, "'")
                ild = float(annot[index[2]+1:index[3]-2])
                correct = data_trial['correct']
                easy = data_trial['cond']
                subj = data_trial['subject']
                # rt = data_trial['rt']
                if ild != 0.05: # only a few followup runs included 1 us
                    ...
                    counters_single[np.where(ilds == ild)[0][0]] += 1
                    counters_all[np.where(ilds == ild)[0][0]] += 1
                    # counting the number of correct trials
                    if correct:
                        corrects_single[np.where(ilds == ild)[0][0]] += 1
                        corrects_all[np.where(ilds == ild)[0][0]] += 1
                    if easy == 1:
                        counter_easy += 1
                        if correct:
                            correct_easy += 1
        ...
        data_fit[si] = np.stack((20*np.log10(ilds), corrects_single, counters_single), axis = -1)
        if score_easy < 0.75:
...
#
##########################################################
ilds = np.array([0.1, 0.2, 0.4, 0.8, 1.6, 3.2])
jsonName = 'ILD_results.json'
f = open(jsonName, 'r')
data = json.load(f)
# the main function for extraction data of interests from the variable—data
data_fit, subjNames, subj_bad, scores_all = sortData(subjList, data, ilds)
figure(ilds, scores_all)   
scipy.io.savemat('data_ILD.mat', dict(data = data_fit, subjNames = subjNames))
f.close
