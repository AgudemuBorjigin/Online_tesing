...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul 21 18:34:15 2020
...
folderName_URL = 'demo_2' # 'demo', 'main', 'demo_2'
folderName_local = 'wavFilesDemo2' # 'wavFiles'， 'wavFilesDemo2' 
jsonName = 'trials_demo_FM_2.json' # 'trials_demo_FM.json', 'trials_main_FM.json', 'trials_demo_FM_2.json'
taskType = 'd_2' # 'd', '', 'd_2'

data = {}
if taskType == 'd':
    data['instructions'] = ["You will be listening to two sound beeps, one of which has fluctuations. <p></p>Your task is to click on either <strong>first</strong> or <strong>second</strong> to indicate in which sound beep you heard the fluctuation.", "When you are ready, click <strong>Continue</strong>."]
elif taskType == 'd_2':
    data['instructions'] = ["When you are ready, click <strong>Continue</strong>."]
else:
    data['instructions'] = ["This section is an extended version of the previous task, which is clicking on <strong>first</strong> or <strong>second</strong> to indicate which sound beep has fluctuation. <p></p>In cases where you can not tell the fluctuation, just give your best guess.", "When you are ready, click <strong>Continue</strong> to hear the first two sound beeps"]
data['feedback'] = True
...
if taskType == 'd':
    data['volume'] = [{
        ...
        "stimulus": "https://speech-in-noise-test.s3.us-east-2.amazonaws.com/audios/BinauralFM/demo/volume.wav",
    ...
    data['volume'] = [{
        ...
        "plugin": "hari-audio-button-response",
        "prompt": "Now adjust your computer volume up to a comfortable (but not too loud) level.",
        "stimulus": "https://speech-in-noise-test.s3.us-east-2.amazonaws.com/audios/BinauralFM/demo_2/volume.wav",
        "choices": ["I have adjusted the volume, let's continue"]
        }]
else:
    data['volume'] = [{
...
fileURLroot = "https://speech-in-noise-test.s3.us-east-2.amazonaws.com/audios/BinauralFM/" + folderName_URL + "/"
fileDirCSV = "/Users/Agudemu/Dropbox/Lab_SNAP/Experiment/stimulus-TFS/stage-2/peripheral/binauralFM/" + folderName_local + "/fileNames.csv"
...
with open(fileDirCSV) as csv_file:
    ...
    for row in csv_reader:
        if row_count != 0:
            ...
            if float(fileName[0:index[0]-2]) > 3:
                cond = 1
            else:
                cond = 2
            data['trials'].append({
                    'plugin': "hari-audio-button-response",
                    'prompt': "Which one had the fluctuation?",
                    'choices': ['first', 'second'],
                    'answer': int(fileName[index[0]+1]),
                    'stimulus': fileURLroot + fileName,
                    'cond': cond,
                    'annot': {'fdev': fileName[0:index[0]]}
        })
        row_count += 1    
